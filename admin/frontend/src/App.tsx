...
const AdminApp: React.FC = () => {
    ...
    const cleanupEmptyFolders = (nodes: BookmarkNode[]): BookmarkNode[] => {
        return nodes.filter(node => {
            if (node.type === 'folder') {
                // 1. 先递归处理子节点
                if (node.children) {
                    node.children = cleanupEmptyFolders(node.children);
                }
                // 2. 如果子节点处理完后变为空了，则该文件夹应该被过滤掉（删除）
                return node.children && node.children.length > 0;
            }
            // 3. 链接节点始终保留（除非它们在之前的逻辑中已被删除）
            return true;
        });
    };
    ...
    const handleDeleteBookmark = (record: FlattenedBookmark) => {
        ...
        // 第二步：关键修复 —— 清理产生的空文件夹
        newBookmarks.links = cleanupEmptyFolders(newBookmarks.links);
        setBookmarks(newBookmarks)
        setIsBookmarksDirty(true)
    }
                        ...
                        {text.split(' / ').map((p, i) => (
                            <React.Fragment key={i}>
                                ...
                                {i < text.split(' / ').length - 1 &&
                                    <ArrowRightOutlined style={{fontSize: 10, color: '#ccc'}}/>}
                            </React.Fragment>
                        ))}
                    </Space>
                )
            ...
            <Card title="安全设置" extra={<Badge status={config?.hasPassword ? 'success' : 'warning'}
                ...
                <Row gutter={24}>
                    <Col span={12}>
                        ...
                        <Text type="secondary">设置或修改后台管理系统的访问密码。密码将加密存储在 GitHub
                            配置文件中。</Text>
