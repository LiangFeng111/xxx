...
const AdminApp: React.FC = () => {
                ...
                render: (_: any, record: any) => (
                    <Space>
                        <Button type="text" icon={<EditOutlined/>}
                                onClick={() => openDataModal(record, record.originalIndex)}/>
                        <Popconfirm title="确定删除该链接吗？"
                                    onConfirm={() => handleDeleteDataLink(record.originalIndex)}>
                      ...
                      extra={
                          <Space>
                              ...
                              <Button type="primary" icon={<SaveOutlined/>} onClick={handleSaveData} loading={loading}
                                      disabled={!isDataDirty}>保存更改</Button>
                          </Space>
                      }>
                    <Row gutter={[24, 24]}>
                        <Col xs={24} md={8}><Text type="secondary">站点标题</Text><Input
                            value={data.settings.site_title} onChange={e => {
                            setData({...data, settings: {...data.settings, site_title: e.target.value}});
                            setIsDataDirty(true)
                        }} style={{marginTop: 8}}/></Col>
                        <Col xs={24} md={8}><Text type="secondary">站点副标题</Text><Input
                            value={data.settings.site_subtitle} onChange={e => {
                            setData({...data, settings: {...data.settings, site_subtitle: e.target.value}});
                            setIsDataDirty(true)
                        }} style={{marginTop: 8}}/></Col>
                        <Col xs={24} md={8}><Text type="secondary">站点公告</Text><Input
                            value={data.settings.site_notice} onChange={e => {
                        ...
                        <Space>
                            ...
                            {selectedDataLinks.length > 0 && (
                                <Button danger size="small" icon={<DeleteOutlined/>}
                                        onClick={handleBulkDeleteDataLinks}>
                                    批量删除 ({selectedDataLinks.length})
                                </Button>
                            )}
                        </Space>
                    }
                    extra={<Button type="dashed" icon={<PlusOutlined/>}
                                   onClick={() => openDataModal()}>添加链接</Button>}
                >
                    <Table
    ...
    const renderBookmarksSection = () => {
        const columns = [
            ...
            },
            {
                ...
                render: (text: string) => (
                    <Space size={[0, 4]} wrap>
                        {text.split(' / ').map((p, i) => (
                            <React.Fragment key={i}><Tag color="blue">{p}</Tag>{i < text.split(' / ').length - 1 &&
                                <ArrowRightOutlined style={{fontSize: 10, color: '#ccc'}}/>}</React.Fragment>
                        ))}
                    </Space>
                )
            },
        ...
        return (
            <Card
                title={<span><BookOutlined/> 书签数据管理 {isBookmarksDirty && <Badge status="warning" text="未保存"/>}</span>}
                extra={<Space wrap>
                    ...
                    <Select placeholder="按文件夹筛选" style={{width: 200}} allowClear
                            options={uniqueFolders.map(f => ({label: f, value: f}))}
                            onChange={value => setBookmarkFolderFilter(value)} value={bookmarkFolderFilter}/>
                    <Input placeholder="搜索书签内容..." prefix={<SearchOutlined/>} value={bookmarkSearch}
                           onChange={e => setBookmarkSearch(e.target.value)} style={{width: 250}} allowClear/>
                    <Button type="primary" icon={<SaveOutlined/>} onClick={handleSaveBookmarks} loading={loading}
                            disabled={!isBookmarksDirty}>保存更改</Button>
                    <Button icon={<PlusOutlined/>} onClick={() => openBookmarkModal()}>添加书签</Button>
                </Space>}>
                <Table dataSource={filteredBookmarks.map(b => ({...b, key: b.id}))} columns={columns} pagination={{
                ...
                render: (_: any, record: any) => (
                    <Space>
                        <Button type="text" icon={<EditOutlined/>} onClick={() => {
                            setEditingHideLink(record);
                            setEditingHideIndex(record.originalIndex);
                            hideForm.setFieldsValue(record);
                            setIsHideModalOpen(true);
                        }}/>
                        <Popconfirm title="确定删除该链接吗？" onConfirm={() => {
                            const newLinks = [...hideData.links];
                            newLinks.splice(record.originalIndex, 1);
                            setHideData({...hideData, links: newLinks});
                            setIsHideDataDirty(true);
                        }}>
                            <Button type="text" danger icon={<DeleteOutlined/>}/>
                  ...
                  extra={
                      <Space>
                          <Input
                              ...
                              allowClear
                          />
                          <Button type="primary" icon={<SaveOutlined/>} onClick={handleSaveHideData} loading={loading}
                                  disabled={!isHideDataDirty}>保存更改</Button>
                          <Button type="dashed" icon={<PlusOutlined/>} onClick={() => {
                              setEditingHideLink(null);
                              setEditingHideIndex(-1);
                              hideForm.setFieldsValue({category: '私密', title: '', url: 'https://', icon: 'link'});
                              setIsHideModalOpen(true);
                          }}>添加隐藏链接</Button>
                      </Space>
                  }>
                <Text type="secondary" style={{marginBottom: 16, display: 'block'}}>提示：此处管理的数据存储在
                    `hide_data.json` 中，在首页输入管理员密码后可见。</Text>
                <Table
        ...
        const columns = [
            {title: '时间', dataIndex: 'timestamp', key: 'timestamp', width: 200},
            {
                ...
                render: (text: string) => <Tag
                    color={text.includes('失败') ? 'red' : text.includes('成功') ? 'green' : 'blue'}>{text}</Tag>
            },
            ...
            {title: '来源', dataIndex: 'ip', key: 'ip', width: 150},
            {
                ...
                render: (_: any, record: any) => (
                    <Space>
                        <Button type="text" icon={<EditOutlined/>}
                                onClick={() => openLogModal(record, record.originalIndex)}/>
                ...
                <Space>
                  ...
                  extra={
                      <Space>
                          ...
                          <Button type="primary" icon={<SaveOutlined/>} onClick={handleSaveLogs} loading={loading}
                                  disabled={!isLogsDirty}>保存更改</Button>
    ...
    const renderSecuritySection = () => (
        <Card title="安全设置" extra={<Badge status={config?.hasPassword ? 'success' : 'warning'}
                                             text={config?.hasPassword ? '已设置访问密码' : '未设置访问密码'}/>}>
            <Row gutter={24}>
                <Col span={12}>
                    <Title level={5}>修改访问密码</Title>
                    <Text type="secondary">设置或修改后台管理系统的访问密码。密码将 SHA-256 加密后存储在 GitHub 的
                        `admin_config.json` 中。</Text>
                    <Form form={passwordForm} onFinish={handleSetPassword} layout="vertical" style={{marginTop: 24}}>
                        ...
                        <Form.Item><Button type="primary" htmlType="submit" loading={loading}
                                           icon={<SaveOutlined/>}>更新密码并同步</Button></Form.Item>
                    </Form>
                </Col>
            </Row>
        </Card>
                ...
                <Card title="管理员登录" style={{width: 400}}>
                    <Form form={loginForm} onFinish={handleLogin} layout="vertical">
                        <Form.Item name="password" label="访问密码"
                                   rules={[{required: true, message: '请输入访问密码'}]}><Input.Password
                            prefix={<SettingOutlined/>} placeholder="请输入访问密码"/></Form.Item>
                        <Form.Item><Button type="primary" htmlType="submit" block
                                           loading={loading}>登录</Button></Form.Item>
                        {!config?.hasPassword &&
                            <Text type="secondary">初次使用？登录后请立即前往安全设置设定密码。</Text>}
                    </Form>
                </Card>
            </div>
        )
    ...
    return (
        <Layout style={{minHeight: '100vh'}}>
            <Sider theme="light" breakpoint="lg" collapsedWidth="0"
                ...
                }}><Title level={4} style={{margin: 0, color: token.colorPrimary}}>Admin</Title></div>
                <Menu mode="inline" selectedKeys={[activeKey]} onClick={handleMenuClick} items={menuItems}
                      style={{borderRight: 0, marginTop: 16}}/>
                <div style={{position: 'absolute', bottom: 16, width: '100%', padding: '0 16px'}}><Button danger block
            ...
            {/* 书签新增/编辑弹窗 */}
            <Modal title={editingBookmark ? '编辑书签' : '添加新书签'} open={isModalOpen}
                   onOk={handleBookmarkModalSubmit} onCancel={() => setIsModalOpen(false)} okText="确认保存 (本地)"
                   cancelText="取消" destroyOnClose>
                <Form form={form} layout="vertical" style={{marginTop: 24}}>
                    <Form.Item name="title" label="标题" rules={[{required: true, message: '请输入标题'}]}><Input
                        placeholder="书签标题"/></Form.Item>
                    <Form.Item name="url" label="URL" rules={[{required: true, message: '请输入 URL'}]}><Input placeholder="https://..."/></Form.Item>
                    <Form.Item name="path" label="文件夹路径 (使用 / 分隔)"
                               rules={[{required: true, message: '请输入路径'}]}><Input
                        placeholder="例如: 常用 / 工具 / 开发"/></Form.Item>
            ...
            >
                <Form form={bulkMoveForm} onFinish={handleBulkMoveBookmarks} layout="vertical" style={{marginTop: 24}}>
                    <Form.Item name="path" label="目标文件夹路径 (使用 / 分隔)"
